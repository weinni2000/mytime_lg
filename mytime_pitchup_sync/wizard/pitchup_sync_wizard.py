...
class PitchupSyncWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        products = dict(DEFAULT_PITCH_TYPE_PRODUCTS)
        products.update({item.pitch_type_id: item.product_id.id for item in mappings})
        values["line_ids"] = [
            (0, 0, {"pitch_type_id": pitch_type_id, "product_id": product_id})
            for pitch_type_id, product_id in products.items()
